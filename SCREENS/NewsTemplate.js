...
export default function NewsTemplate(props) {
    ...
    return (
        <View style={styles.container}>
            <Image source={{ uri: (props?.urlToImage != undefined) ? props.urlToImage : `https://upload.wikimedia.org/wikipedia/commons/d/d1/Image_not_available.png?20210219185637` }} style={styles.Image} />
            <Text style={styles.title}>{props.title}</Text>
            <Text style={styles.description}>{props.description}</Text>
            <View style={styles.data}>
                <Text style={{ color: 'white' }} >BY <Text style={{ color: 'green' }}>{props.author}</Text></Text>
